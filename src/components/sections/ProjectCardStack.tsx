      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4 sm:gap-6">
            <span className="text-[#D7E2EA]/20 font-black leading-none" style={{ fontSize: 'clamp(2.5rem, 8vw, 100px)' }}>
              {String(index + 1).padStart(2, '0')}
            </span>
            <div className="flex flex-col">
              {p.category && <span className="text-[#D7E2EA]/60 uppercase tracking-widest text-xs sm:text-sm">{p.category}</span>}
              <h3 className="text-[#D7E2EA] font-medium uppercase" style={{ fontSize: 'clamp(1.2rem, 3vw, 2.4rem)' }}>{p.title}</h3>
            </div>
          </div>
          {p.live_url && <LiveProjectButton href={p.live_url} />}
        </div>
        ...
        <p className="text-[#D7E2EA]/70 text-sm sm:text-base max-w-xl">{p.description}</p>
      </motion.div>
    </div>
  );
}
